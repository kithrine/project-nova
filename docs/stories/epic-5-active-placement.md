...
- Implement `ADR-017` Layer 2 independently from portable Story 3.4 training: record host confirmation of actual-site hazard/emergency orientation, chemicals/SDS and PPE instruction where applicable, local sanitation SOPs/equipment, assigned-task restrictions, and supervisor-observed competency where required.
...
7. Given all portable Story 3.4 training is complete but host-site instruction or required supervisor competency confirmation is missing, when activation blockers are evaluated, then Placement onboarding remains incomplete.
...
Task generation is tied to the placement reaching Approved and entering Onboarding. Task completion does not itself transition the placement — activation (5.6) is a separate, explicit action gated by the blocker list. Story 3.4 completion never auto-completes this site-specific layer. Tasks are never hard-deleted.
