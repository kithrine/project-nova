...
- Track status (e.g., Enrolled, In Progress, Completed, Withdrawn) and relevant dates (enrolled date, expected/actual completion date).
- Allow the coordinator to update status and dates as training proceeds.
...
2. Given a Training Enrollment, when its status is updated (e.g., to Completed), then the change and the date are recorded, and history is preserved rather than overwritten.
...
### Authorization
Create/update requires `trainingEnrollment.create` / `trainingEnrollment.update` scoped to Nova Operations membership. Participants have no direct write access; their visibility is limited to the rolled-up journey step, not the Training Enrollment records themselves.

### Lifecycle rules
Represents the participant's Training stage in `docs/product/participant-lifecycle.md`. Training Enrollment status changes are historical and captured only through recorded transitions (e.g., Withdrawn), never silently edited.

### Data changes
Creates/updates `Training Enrollment` (linked to `Participant`/`Program Enrollment` and `Training Program`). Assumes `Training Program` records exist as configured/seeded catalog data for MVP (authoring the catalog is not part of this story).
...
### Tests
- Unit: status-transition rules for Training Enrollment; readiness-input mapping.
- Integration: history is preserved across status updates (no destructive overwrite); organization/role scoping on writes.
...
### Out of scope
Certification records (3.5), a dedicated participant-facing training management page, and authoring the Training Program catalog itself.
...
2. Given a Program Enrollment with an incomplete or expired required Training Enrollment or Certification, when the blocker list is computed, then each one appears as a blocker.
