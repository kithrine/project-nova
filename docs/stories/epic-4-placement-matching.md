...
### User story
As a Program Coordinator, I want to give final approval to a match once both the participant and the shelter have agreed, so that an approved Placement is created and the participant can move into placement onboarding.
...
- Create the `Placement` starting at Approved status. The Draft/Proposed/Shelter Review stages named in `docs/product/placement-lifecycle.md`'s Placement lifecycle diagram are satisfied by the match's own Draft → Proposed → decision cycle (`docs/decisions/ADR-002-separate-lifecycles.md` keeps Placement Match and Placement distinct precisely so pre-placement states don't duplicate onto placement data); the created Placement is handed to Epic 5 already at Approved, ready for onboarding. See Out of scope for the flagged documentation gap this resolves.
...
### Acceptance criteria
1. Given a Proposed match with `participantDecision` = Accepted and `shelterDecision` = Approved, when the coordinator approves the match, then it transitions to Approved and a new Placement is created in the same transaction, linked to the match.
...
### Out of scope
Placement onboarding, activation, schedule confirmation, and funding assignment finalization (Epic 5, Stories 5.2–5.6); reporting on match-to-placement conversion (Epic 7). Flagged for the team: `docs/product/placement-lifecycle.md`'s "Placement lifecycle" diagram lists Draft, Proposed, and Shelter Review as stages internal to Placement itself, ahead of Approved — no story in this epic or in Epic 5 implements a second Draft/Proposed/Shelter-Review cycle on the Placement, and "Shelter Review" appears nowhere else in the documentation set. This story resolves the apparent duplication by treating those early stages as satisfied by the match's own lifecycle and creating the Placement directly at Approved; the diagram should be reconciled with ADR-002 in a documentation follow-up.
