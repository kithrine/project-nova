...
A program-scoped catalog definition for portable learning that can be required before matching. It is not a credential and does not authorize site-specific work.

## Training Enrollment

One Program Enrollment's attempt to complete one Training Program. It moves through Enrolled, In Progress, Completed, or Withdrawn with append-only history. A later attempt is a new record.

## Portable Training

General preparation completed before matching that remains useful across host sites. It does not replace host site-, hazard-, equipment-, or assignment-specific instruction and competency confirmation.

## Site-Specific Competency

The host's documented confirmation that a participant received instruction for the actual site and assigned work and, where required, demonstrated the skill under supervision. It is a Placement onboarding gate.
